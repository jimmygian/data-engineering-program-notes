...
The languagd follows a simple pattern where you start with the **keyword** `resource`, then you specify the **resource type** and the **name** you want to give this resource. 
...
```
- Write the configuration files to define your resources, 
- Ask Terraform to prepare your workspace. 
	- It installs the necessary files that enable 
	  communication with the cloud APIs 
	- It creates an execution plan describing 
...
- It starts with a **keyword** that tells you what **"type"** of block it is (block type).
	- e.g. `resource`, `provider`, `variable` etc..
...
- You can find all the available providers in the Terraform registry https://registry.terraform.io/. 
	- Some providers allow Terraform to interact with the cloud platform, 
	- Others are utilities providers that allow you to use additional functionalities in Terraform. 
...
- The local name is the unique identifier that you can use everywhere in the configuration file to refer to the provider. For example, here I'm using `aws` as a local name for the AWS provider, which is the preferred name that is used in terraform's AWS provider documentation. 
...
What you see in the configuration file above is enough to create an EC2 instance. So let's actually create this EC2 instance. 

I'm going to start with the Terraform init command. 
...
In this example, under the `# input` section, I'm going to create one variable that represents the region name, and another one that represents the server's name. You can declare these variables like this using the `variable` keyword, and then, the preferred variable `<name>`. 
...
- Here the strings `region` and `server_name` represent the name of each variable, which acts as *identifiers* for the variables inside the Terraform configuration file. 
...
As mentioned above, if you don't declare a default name for your variable, you'll be prompted to provide that value when running `terraform plan` and `terraform apply`.

To avoid this, you can **automatically** assign values to the variables without being prompted by using the command line flag `–var` in terminal:
```shell
terraform apply -var server_name=ExampleServer	
```

Even more conveniently, you can **define** the values of your variables in a separate file that has `.tfvars` as its extension. 
...
- Now, to update the file, you can just use the command `terraform apply` without specifying any variables directly in the terminal. Terraform will use that `<variables>.tfvars` file to extract the values of the variables and update the configuration with the provided values. 


##### Exporting values
Now, any **resource** that you create will contain a list of characteristics or attributes. 

>	For example, you can check the documentation of the EC2 instance.  
>	https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/instance#attribute-reference

You can see a list of all of its attributes, such as 
- the id instance, 
- its ARN, which stands for Amazon Resource Name, 
- and its public IP address.
![[Screenshot 2025-12-10 at 19.30.25.png]]![[Screenshot 2025-12-10 at 19.34.28.png]]
...
So, let's create two output values to export: the id and ARN of the EC2 instance we are creating. 
...
- The strings that come after the `output` keyword represent the names (identifiers) for these output values. 
- For each output value, you need to specify the `value` argument by assigning it to the attribute of the EC2 instance. 

You can access the attributes of a resource by specifying the identifier for the resource, which is
...
>	Again, we know that our `aws_instance` contains an `id` and an `arn` attribute because of the terraform's [aws_instance](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/instance#attribute-reference) documentation -> Attribute Reference section.
...
### Organizing our Terraform Project
Right now, I have several blocks in one configuration file where each block has a different purpose. But what if you have more than one resource with several input variables and several output values? If you declare all the blocks in one file, then at some point managing the file will become very cumbersome. 

> The better practice is to **split** this configuration file into several files. 

A classic splitting configuration is:
...
Terraform will *automatically* concatenate all the files of the folder that end with `.tf` as if you've written all of them in one file. It will also check the `<file>.tfvars` file to extract any variable values declared.
...
In addition to the *resource* variable and *output* blocks that you saw in the previous sections, you can also declare **"data"** blocks in Terraform. By using Data blocks in your configuration file, you can **reference resources created outside of Terraform or in another Terraform workspace**. 
...
So, we can now reference our data source's `id` inside our resource block:
...
In the first section, I explained that I grabbed this AMI from the AWS console. If I want to automate this process by asking Terraform to do the search on my behalf and retrieve the latest Linux AMI, I can use the following data block. 
...
>	*A module is like a **regular root directory**, so it also expects you to declare 
>	- the providers, 
>	- the input variables, 
>	- and the output values inside of it. 
>So I need to move those files into the website module.* 

![[Screenshot 2025-12-12 at 13.48.52.png]]

The *module* we created is a **self-contained folder** (kind of like a mini-terraform project) that contains the resource, the input variables, and output values for that module. So, we can't directly access this information in the root directory, meaning that if you run `terraform apply` now, it will give you errors because the root directory won't be able to *see* the values assigned to the input variables. 
...
Here `[..]` I need to specify a value for the input variable `server_name`. And for that, I'm going to create this `variables.tf` file in the main directory and declare the `server_name_root` variable. 
...
And finally, if you want to export the module's outputs `server_id` and `server_arn` also as outputs in the root directory, you need to create an `outputs.tf` file inside the root directory. 
- In this file, you declare the outputs and assign them to the values of the module's outputs using this syntax: 
	- You start with the word `module`, 
	- then you specify the `name` of the module that contains these output values. 
	- And then you specify the name of the `output values` you want to access. 
